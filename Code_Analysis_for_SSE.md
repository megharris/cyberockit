...
Our team has become a well-oiled machine at this point in the semester. When this project was released, we immediately met and divided up the work among ourselves with a plan to reconnect after the holiday. However, as is often the case, several personal situations developed that caused us to need to adapt and adjust on the fly. We re-evaluated the division of work to compensate for those team members who were unable to commit hours at a particular point in time. Then as mentioned above in the code review strategy, we realized that gaining familiarity of the CWEs caused us to adjust our strategy multiple times. We first identified CWEs, then quickly became overwhelmed by the sheer volume of code to review and the extreme lack of knowledge of Python to understand if there were coding bugs. Many, but not all, of us had enough familiarity to be able to understand what the code was attempting to achieve but it was INCREDIBLY difficult for us to determine if the security was existent, let alone functional. We collaborated well by asking each other for assistance in determining the controls that should be in place as well as identifying them in the code. We all agree that we would be happy to work togehter again in the future if some of our members were not graduating this smeseter.
