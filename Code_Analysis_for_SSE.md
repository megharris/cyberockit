...
All 8 vulnerabilities are marked as critical/high severity and mapped to 4 different CWE’s.
...
[CWE-327](https://cwe.mitre.org/data/definitions/327.html), [CWE-326](https://cwe.mitre.org/data/definitions/327.html) & [CWE-295](https://cwe.mitre.org/data/definitions/295.html):

Again, 3 out of 6 vulnerabilities mapped to these CWE’s belong to the tests folder which is not relevant to our review process. The other 3 vulnerabilities are supposedly at risk because of broken or untrusted cryptography usage. However, upon inspection we found that the problem arised because the method used has a newer available version with updated features that could be implemented by HA for better security.
...
The 3 high severity issues are mapped to CWE-23. If an attacker can get hands on the contents of final_path the core system can be compromised. This issue is found in the [components](https://github.com/home-assistant/core/tree/dev/homeassistant/components) folder of HA core.  
...
We collaborated well by asking each other for assistance in determining the controls that should be in place as well as identifying them in the code. We all agree that we would be happy to work together again in the future if some of our members were not graduating this semester.

## Pull Request
Based on CWE 291, [here](https://github.com/home-assistant/core/pull/62236).
